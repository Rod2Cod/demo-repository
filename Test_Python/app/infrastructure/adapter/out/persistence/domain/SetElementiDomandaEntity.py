...
elementi_set = db.Table('elementi_set', db.Model.metadata,
    db.Column('id_elemento', db.Integer, db.ForeignKey('elemento_domanda.id'), primary_key=True),
    db.Column('nome_set', db.Text, db.ForeignKey('set_elementi_domanda.nome'), primary_key=True)
)